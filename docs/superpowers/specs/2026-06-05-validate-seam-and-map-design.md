...
- `mavenPublishing { }` block with updated coordinates: `io.github.sahsenvar:kmapper-validators`, name `"kmap-validators"`, description `"Pre-built Validator<T> implementations for kmap"`.
...
- Value type `V1 → V2` is either directly-assignable (same type) OR nested-mappable (`V1` has `toV2()` generated by kmap).
